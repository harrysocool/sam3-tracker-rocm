...
6. **UMA BIOS setting (64 GB)**: The Ryzen AI Max+ 395 is an APU — GPU and CPU share the same physical DRAM. The BIOS "UMA Frame Buffer Size" controls how much of that RAM is reserved as GPU-visible VRAM. Setting it to 64 GB (all available memory) measurably improves throughput for memory-bandwidth-limited workloads: +2.4% on the encoder+decoder chain path and up to +20.7% on the fused FP16 path were recorded during early benchmarking. The tracker backbone is similarly memory-bandwidth-limited, so UMA=64 GB is the recommended setting. The current machine is already configured at this value (`rocm-smi` reports 64 GB VRAM).
