...
6. **UMA BIOS setting (64 GB)**: The Ryzen AI Max+ 395 is an APU — GPU and CPU share the same physical DRAM. The BIOS "UMA Frame Buffer Size" determines how large a **coarse-grained (non-coherent)** memory pool is carved out for the GPU at boot. Memory inside this pool is fast for GPU access; memory outside it is **fine-grained (cache-coherent)**, which is 2–4× slower because every GPU access must go through CPU cache coherency logic. With our 64 GB machine set to UMA=64 GB, the entire RAM pool is coarse-grained, maximising effective GPU memory bandwidth. Measurable impact in early benchmarking: +2.4% on the encoder+decoder chain path, up to +20.7% on the fused FP16 path. The tracker backbone is memory-bandwidth-limited, so this setting matters. The current machine is already configured at 64 GB (`rocm-smi` reports 64 GB VRAM).
