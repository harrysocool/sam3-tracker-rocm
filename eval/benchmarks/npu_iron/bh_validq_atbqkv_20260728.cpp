...
static const int C=1024,d=64,nH=16,Hid=4736,Hpad=5120,Nhalf=2560,MFFN=1536,MATB=2048,GRID=36,S_G=1296,Sp_ln=1344;
...
// global handles
H hln,hqkv_w,hqkv_g,hatbq,ho_w,ho_g,hqt_w,hsm_w,hpv_w,hgelu,hf2,hf1v2,hflashw,hflashg;
// LN bos
xrt::bo lin,lout;
// proj scratch
...
void cpu_ln_snapshot(const vector<float>&x,const vector<float>&w,const vector<float>&b,vector<float>&out,vector<float>&res){
  // Snapshot the residual while the row is hot, then produce FP32 LayerNorm.
  int S=x.size()/C; out.resize(S*C); res.resize(S*C);
  #pragma omp parallel for schedule(static)
  for(int s=0;s<S;s++){ const float*xr=&x[s*C]; float*orow=&out[s*C];
    float*rr=&res[s*C]; float mean=0.f;
    for(int cc=0;cc<C;cc++){ float v=xr[cc]; rr[cc]=v; mean+=v; } mean/=C;
...
void attn(const float*qkv,const vector<float>&bq,bool glob,xrt::bo&oA){
  ...
  if(!glob){ // window flash attention: fused qkt+softmax+pv, 1 dispatch, Q unscaled (flash folds 1/sqrt(d))
    ...
    // Partial windows use compact Q rows (288/288/144, padded by the runtime
    // schedule to 384/384/192). K/V retain all 576 positional rows.
    std::memset(Qf,0,(size_t)64*576*d*sizeof(bf16));
    ...
    { // Split+bias+RoPE+BF16 in one pass directly into the flash input BOs.
      ...
      _Pragma("omp parallel for schedule(static)") for(int t=0;t<2304;t++){
        ...
        bool valid=gi<GRID&&gj<GRID;
        const float*qrow=valid?qkv+((size_t)gi*GRID+gj)*3*C:zero_qkv.data();
        const float*cos_s=co+p*d, *sin_s=si+p*d;
        for(int h=0;h<nH;h++){
          int g=win*nH+h; size_t kvout=((size_t)g*S+p)*d;
          int compact_p=i*(w1?12:24)+j;
          size_t qout=((size_t)g*S+compact_p)*d;
        for(int i=0;i<d;i+=2){
          float qi=qrow[h*d+i]+bq[h*d+i], qi1=qrow[h*d+i+1]+bq[h*d+i+1];
          float ki=qrow[C+h*d+i]+bq[C+h*d+i], ki1=qrow[C+h*d+i+1]+bq[C+h*d+i+1];
          if(valid){
            Qf[qout+i]  =f2b(qi*cos_s[i]  -qi1*sin_s[i]);
            Qf[qout+i+1]=f2b(qi1*cos_s[i+1]+qi*sin_s[i+1]); }
          Kf[kvout+i]  =f2b(ki*cos_s[i]  -ki1*sin_s[i]);
          Kf[kvout+i+1]=f2b(ki1*cos_s[i+1]+ki*sin_s[i+1]); }
          for(int c=0;c<d;c++) Vf[kvout+c]=f2b(qrow[2*C+h*d+c]+bq[2*C+h*d+c]);
        }
      }
    }
    ...
    _Pragma("omp parallel for schedule(static)") for(int t=0;t<S_G;t++){
      int gi=t/GRID,gj=t%GRID,win=(gi/24)*2+(gj/24);
      int compact_p=(gi%24)*((gj/24)?12:24)+(gj%24);
      for(int h=0;h<nH;h++)for(int c=0;c<d;c++)
        dst[(size_t)t*C+h*d+c]=src[((size_t)(win*nH+h)*576+compact_p)*d+c];
    }
...
void block(vector<float>&x,int li){
  CUR_LAYER=li;
  bool glob=isglob(li);
  double tp=now(); static vector<float> res; vector<float> xn;
  cpu_ln_snapshot(x,ln1w[li],ln1b[li],xn,res); P.ln1+=now()-tp;  // [1296,C]

  // ATB QKV projection is token-wise. Config1 requires M=2048; only the 1296
  // valid output rows are converted back to FP32 for the existing attention
  // packing path.
  H&hq=hatbq; xrt::bo&qA=atbqA; xrt::bo&qCo=atbqC;
  tp=now(); static bool qkv_input_initialized=false;
  wbf_mapped(qA,xn.data(),(size_t)S_G*C,(size_t)MATB*C,qkv_input_initialized);
  P.qkv_pack+=now()-tp;
  DISP_ACC(hq.k(3,hq.bi,hq.nw,qA,WB_atbq[li],qCo),P.qkv_disp,"qkv_atb_m2048");
  tp=now(); qCo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
  const bf16*qkv_bf=qCo.map<const bf16*>();
  static vector<float> qkv_fp((size_t)S_G*3072);
  #pragma omp parallel for schedule(static)
  for(long i=0;i<(long)S_G*3072;i++) qkv_fp[(size_t)i]=b2f(qkv_bf[(size_t)i]);
  const float*qkv=qkv_fp.data(); P.qkv_read+=now()-tp;
  ...
  // Bias + residual directly into x; preserve res + (ao + bias) association.
  tp=now();
  #pragma omp parallel for schedule(static)
  for(int r=0;r<S_G;r++){ const float*rr=&res[r*C],*ar=ao+(size_t)r*C; float*xr=&x[r*C]; int cc=0;
    for(;cc+16<=C;cc+=16){ __m512 attn=_mm512_add_ps(_mm512_loadu_ps(ar+cc),_mm512_loadu_ps(ob.data()+cc));
      _mm512_storeu_ps(xr+cc,_mm512_add_ps(_mm512_loadu_ps(rr+cc),attn)); }
    for(;cc<C;cc++) xr[cc]=rr[cc]+(ar[cc]+ob[cc]); }
  P.unpartition_res1+=now()-tp;

  // ffn
  tp=now(); static vector<float> res2; vector<float> xn2;
  cpu_ln_snapshot(x,ln2w[li],ln2b[li],xn2,res2); P.ln2+=now()-tp;
  ...
  tp=now(); { // Padded W1/bias/W2 regions are exactly zero; only valid GELU values contribute.
    ...
    _Pragma("omp parallel for schedule(static)") for(int r=0;r<S_G;r++){ const float*hr=hid_+(size_t)r*Hpad; bf16*ob=gb+(size_t)r*Hpad;
...
int main(int argc,char**argv){
  ...
  const string S=A+"shared_gemm_candidate_m32n64/";
  hqkv_w=loadx(S+"qkv_w");
  hqkv_g=loadi(hqkv_w,S+"qkv_g");
  hatbq=loadx("/home/amd/project/npu_iron/atb_eval_20260727/chess1p9_config1_qkv_m2048_k1024_n3072");
  ho_w=loadi(hqkv_w,S+"o_w");
  ho_g=loadi(hqkv_w,S+"o_g");
  hf1v2=loadi(hqkv_w,S+"ffn1");
  hflashw=loadx(A+"valid_query_flash_grouped_wip_20260727"); hflashg=loadx(A+"attn_v2/flash_g");
  hf2=loadx(A+"ffn_mc_v2/ffn2");
  ...
  atbqA=mkbo(hatbq,3,(size_t)MATB*C*2); atbqC=mkbo(hatbq,5,(size_t)MATB*3072*2);
  ...
  for(int li=0;li<32;li++){ bool glob=isglob(li); char b[64];
    ...
    auto Wq_shuffled=gemm_atb::layout_transpose_L1_1x2_8x8block(Wq,C,3072,64,128);
    auto Wq_bfp=floatToBfp16(8,(int)Wq_shuffled.size(),Wq_shuffled.data(),0);
    WB_atbq[li]=mkbo(hatbq,4,Wq_bfp.size());
    std::memcpy(WB_atbq[li].map<void*>(),Wq_bfp.data(),Wq_bfp.size());
    WB_atbq[li].sync(XCL_BO_SYNC_BO_TO_DEVICE);
