// SAM3 ViT backbone C++ host (504px), grouped valid-query stall profiler.
...
int CUR_LAYER=-1;
int CUR_FRAME=-1;
long DISPATCH_ORDINAL=0,LAST_SLOW_ORDINAL=-1;
const char*LAST_DESIGN="none";
static string pm_status(){
  std::ifstream f("/sys/bus/pci/devices/0000:c6:00.1/power/runtime_status");
  string s="unavailable"; if(f)f>>s; return s;
}
#define DISP_ACC(call,acc,name,design) do{ \
  long _ord=DISPATCH_ORDINAL++; const char*_prev=LAST_DESIGN; \
  bool _transition=std::strcmp(_prev,design)!=0; double _t=now(); \
  (call).wait(); double _dt=now()-_t; T_disp+=_dt; (acc)+=_dt; LAST_DESIGN=design; \
  if(_dt>20.0){ long _gap=LAST_SLOW_ORDINAL<0?-1:_ord-LAST_SLOW_ORDINAL; \
    string _pm=pm_status(); \
    fprintf(stderr,"slow_dispatch frame=%d ordinal=%ld since_last=%ld layer=%d stage=%s design=%s prev_design=%s transition=%d pm=%s ms=%.3f\n", \
      CUR_FRAME,_ord,_gap,CUR_LAYER,name,design,_prev,_transition?1:0,_pm.c_str(),_dt); \
    LAST_SLOW_ORDINAL=_ord; } \
}while(0)
...
void attn(const float*qkv,const vector<float>&bq,bool glob,xrt::bo&oA){
  ...
  if(!glob){ // window flash attention: fused qkt+softmax+pv, 1 dispatch, Q unscaled (flash folds 1/sqrt(d))
    ...
    P.attn_h2d+=now()-tp;
    DISP_ACC(hflashw.k(3,hflashw.bi,hflashw.nw,qaF,qbF,pbF,OF),P.flash_disp,"flash_w","flash_w");
    tp=now();
  ...
  P.attn_h2d+=now()-tp;
  DISP_ACC(hflashg.k(3,hflashg.bi,hflashg.nw,qaG,qbG,pbG,OFg),P.flash_disp,"flash_g","flash_g");  // flash global → bf16
  tp=now();
...
void block(vector<float>&x,int li){
  ...
  P.qkv_pack+=now()-tp;
  DISP_ACC(hq.k(3,hq.bi,hq.nw,qA,WB_qkv[li],qCo),P.qkv_disp,"qkv_m1536","gemm_k1024");
  ...
  DISP_ACC(ho.k(3,ho.bi,ho.nw,oA,WB_o[li],oCo),P.odisp,"oproj_m1536","gemm_k1024");
  ...
  P.ffn1_pack+=now()-tp;
  DISP_ACC(hf1v2.k(3,hf1v2.bi,hf1v2.nw,f1Av2,WB_w1full[li],f1Cfull),P.ffn1_disp,"ffn1","gemm_k1024");
  ...
  DISP_ACC(hf2.k(3,hf2.bi,hf2.nw,gsh,WB_w2[li],f2C),P.ffn2_disp,"ffn2","ffn2");
...
int main(int argc,char**argv){
  ...
  if(input_file.empty()){
    ...
    while(true){
      ...
      // Run inference
      CUR_FRAME=frame_idx;
      fprintf(stderr,"frame_begin frame=%d ordinal=%ld pm=%s\n",CUR_FRAME,DISPATCH_ORDINAL,pm_status().c_str());
      T_disp=0; P={}; double t0=now();
      for(int li=0;li<32;li++) block(x,li);
      double wall=now()-t0;
      fprintf(stderr,"frame_end frame=%d ordinal=%ld pm=%s wall=%.0fms dispatch=%.0fms\n",CUR_FRAME,DISPATCH_ORDINAL,pm_status().c_str(),wall,T_disp); fflush(stderr);
  ...
  for(int r=0;r<n_runs;r++){ CUR_FRAME=r; T_disp=0; P={}; double t0=now(); x=x0; for(int li=0;li<32;li++) block(x,li); double wall=now()-t0;
