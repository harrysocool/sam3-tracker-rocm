...
void attn(vector<float>&q,vector<float>&k,vector<float>&v,bool glob,xrt::bo&oA){
  ...
  if(!glob){ // window flash attention: fused qkt+softmax+pv, 1 dispatch, Q unscaled (flash folds 1/sqrt(d))
    double tp=now();
    static vector<bf16> Qf,Kf,Vf; Qf.assign(G*S*d,0);Kf.assign(G*S*d,0);Vf.assign(G*S*d,0);
    ...
    _Pragma("omp parallel for schedule(static)") for(int t=0;t<2304;t++){ int win=t/576,p=t%576; for(int h=0;h<nH;h++)for(int c=0;c<d;c++) dst[t*C+h*d+c]=src[((win*nH+h)*576+p)*d+c]; }
...
void block(vector<float>&x,int li){
  ...
  tp=now(); static vector<float> xflat;
  ...
  // RoPE and flash output layout are applied directly into the BF16 O-projection input BO.
  attn(q,k,v,glob,oA);
  // oproj
  ...
  // +b1 + gelu -> gin bf16
  tp=now(); { // Tier3: bias+gelu+f2b fused on CPU — skips NPU gelu dispatch (~85ms → ~19ms)
