// SAM3 ViT backbone C++ host (504px), projection-compaction candidate.
...
void block(vector<float>&x,int li){
  ...
  tp=now(); static vector<float> xin; xin.assign(MFFN*C,0.f); std::memcpy(xin.data(),xn.data(),S_G*C*4);
  wbf_v(qA,xin);
  P.qkv_pack+=now()-tp;
