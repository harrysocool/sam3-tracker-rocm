...
OUT=$ROOT/sam3_attn/valid_query_flash_q64_shapes_20260726
LOG=/home/amd/project/9_to_delete/git_cleanup_20260721/manifests/valid_query_flash_q64_shapes_20260726_build
QUAR=/home/amd/project/9_to_delete/valid_query_flash_q64_stale_20260726
...
make -f Makefile compile-kernel \
  LK=576 LQ=576 LKP=64 LQP=192 DK=64 DV=64 \
  NUM_Q_TILES=3 PEANO_INSTALL_DIR="$PEANO_INSTALL_DIR" \
  >"$LOG/compile_kernel_q64.log" 2>&1
...
quarantine_stale() {
  ...
  for path in air.xclbin air.insts.bin air.elf air_project; do
    if [[ -e "$BUILD/$path" ]]; then
      ...
      if [[ -e "$target" ]]; then target=${target}_$(date +%H%M%S%N); fi
...
generate() {
  ...
  (
    ...
    python3 "$SRC/attn_npu2.py" \
      --lk 576 --lkp 64 --lq "$lq" --lqp 192 --dk 64 --dv 64 \
  ...
  printf 'LQ=%s LK=576 LQP=192 tile_q=64 heads=16\n' "$lq" >"$target/config.txt"
  sha256sum "$target/final.xclbin" "$target/insts.bin"
}

generate q576 576
generate q384_for_q288 384
generate q192_for_q144 192

echo VALID_QUERY_FLASH_Q64_SHAPES_DONE
